...
export const fetchNotionAgenda = async (locals?: any) => {
    ...
    if (!secret || !agendaDbId) {
        throw new Error('Missing Notion credentials: NOTION_SECRET and NOTION_AGENDA_DB_ID are required');
    }
    ...
    const agendaItems = response.data.results.map((page: any) => {
        ...
        const moderator = moderatorRelations.length > 0
            ? speakerMap.get(moderatorRelations[0].id)
            : null;
