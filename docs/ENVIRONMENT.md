...
| `MEMBERSHIP_LIVE`              | `getEnv`                     |   ❌ **missing**   |        ❌        |          ❌           | **Dead** — read only by `HomeLayout` and `-new` pages, none of which are reachable |
...
4. **`MEMBERSHIP_LIVE` is dead and can be removed from the Cloudflare dashboard.** It's a plain feature-flag string (`"true"`/anything else), not a secret. It is read only in `HomeLayout.astro` and the `-new` pages, all of which are unreachable since the redesign was shelved in #524 — so no live page reads it and its value has no effect. It also has no record in any env file or doc.
